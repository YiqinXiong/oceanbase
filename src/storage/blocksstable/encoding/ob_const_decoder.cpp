    ...
    if (0 == ref) {
      switch (get_store_class_map()[tc]) {
        ...
        case ObIntervalSC: {
          ...
          ObObjDatumMapType datum_type = ObDatum::get_obj_datum_map_type(ctx.obj_meta_.get_type());
          const uint32_t size = ObDatum::get_reserved_size(datum_type);
          MEMCPY(const_cast<char *>(tmp_datum.ptr_), meta_header_->payload_, size);
          tmp_datum.len_ = size;
          if (OB_FAIL(tmp_datum.to_obj(cell, ctx.obj_meta_))) {
        ...
        switch (filter.get_op_type()) {
          ...
          case sql::WHITE_OP_IN: {
            if (OB_UNLIKELY(objs.count() == 0)) {
              ...
              if (OB_FAIL(filter.exist_in_objs(const_obj, is_existed))) {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
    ...
    if (meta_header_->const_ref_ == dict_count) {
      ...
      if (OB_FAIL(ret)) {
      } else if (OB_FAIL(filter.exist_in_objs(const_obj, const_in_result_set))) {
    ...
    if (OB_SUCC(ret) && dict_count > 0) {
      ...
      if (is_sorted_dict) {
        ...
        if (filter.is_min_max_valid() && (last_dict < min_param || first_dict > max_param)) {
          ...
          right_it = std::upper_bound(left_it, end_it, max_param);
        }
      }

      if (!is_hit_shortcut &&
          OB_FAIL(set_ref_exist_in_objs(left_it, right_it, begin_it,
                                        is_sorted_dict, filter, *ref_bitset,
                                        found, const_in_result_set))) {
...
int ObConstDecoder::set_ref_exist_in_objs(
    ...
    bool &found,
    bool const_in_result_set) const
{
  int ret = OB_SUCCESS;
  ...
  case sql::ObWhiteFilterObjSetCmpType::DUAL_POINTER: {
    ...
    int64_t ref_bitset_size = dict_decoder_.get_dict_header()->count_ + 1;
  ...
  case sql::ObWhiteFilterObjSetCmpType::HASH_SEARCH: {
    ...
    while (OB_SUCC(ret) && dict_it != right_it) {
      ...
      if (OB_SUCC(ret) && !const_in_result_set == is_exist) {
        found = true;
        ref_bitset.set(dict_ref);
      }
      ++dict_it;
