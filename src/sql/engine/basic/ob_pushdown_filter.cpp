...
int ObWhiteFilterExecutor::eval_in_right_val_to_objs()
{
  ...
  if (WHITE_OP_IN != filter_.get_op_type()) {
    ...
    ObDatum *right = NULL;
    null_param_contained_ = false;
    if (!OB_ISNULL(expr.args_[0]) && OB_FAIL(expr.args_[0]->eval(ctx, left))) {
...
{
  int ret = OB_SUCCESS;
  if (min_max_valid_ && (obj < params_.at(min_param_idx_) || obj > params_.at(max_param_idx_))) {
...
int ObWhiteFilterExecutor::exist_in_objs(const ObObj &obj, bool &is_exist) const
{
  int ret = OB_SUCCESS;
  if (obj_array_sorted_ && params_.count() > 64) {
    ret = exist_in_obj_array(obj, is_exist);
  } else {
    ret = exist_in_obj_set(obj, is_exist);
  }
  return ret;
}

ObWhiteFilterObjSetCmpType ObWhiteFilterExecutor::get_obj_set_cmp_type(const int64_t len_dict, bool is_sorted_dict) const {
  // delta in [-1, 1]
  // delta [-1, -0.5)   means b > 3*a
  // delta [-0.5, 0.5]  means b <= 3*a and a <= 3*b
  // delta (0.5, 1]     means a > 3*b
  static constexpr double DELTA_THRES = 0.5;
  static constexpr int64_t HASH_SLOTS = 16;

  sql::ObWhiteFilterObjSetCmpType cmp_type = ObWhiteFilterObjSetCmpType::HASH_SEARCH;
  const int64_t len_objs = params_.count();
  double delta = static_cast<double>(len_dict - len_objs) /
                 static_cast<double>(len_dict + len_objs);
  if (is_sorted_dict) {
    if (delta > DELTA_THRES) {
      // a >> b
      if (len_dict > HASH_SLOTS * 4) {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::BINARY_SEARCH_DICT;
      } else {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::DUAL_POINTER;
      }
    } else if (delta >= -DELTA_THRES) {
      // a ~~ b
      if (len_dict > HASH_SLOTS) {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::DUAL_POINTER;
      } else {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::HASH_SEARCH;
      }
    } else {
      // a << b
      if (len_objs > HASH_SLOTS * 4) {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::BINARY_SEARCH;
      } else if (len_objs > HASH_SLOTS * 3) {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::DUAL_POINTER;
      } else {
        cmp_type = sql::ObWhiteFilterObjSetCmpType::HASH_SEARCH;
      }
...
int ObWhiteFilterExecutor::init_min_max_param_idx()
{
  ...
  if (OB_LIKELY(params_.count() > 0)) {
    if (obj_array_sorted_) {
      min_param_idx_ = 0;
      ...
      for (ParamArrayIter it = params_.begin(); it != params_.end(); ++it) {
        ObObj& cur_obj = *it;
        if (min_param_idx_ == UINT64_MAX || cur_obj < params_.at(min_param_idx_)) {
          min_param_idx_ = cur_idx;
        }
        if (max_param_idx_ == UINT64_MAX || cur_obj > params_.at(max_param_idx_)) {
    ...
    if (min_param_idx_ == UINT64_MAX || max_param_idx_ == UINT64_MAX) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("Failed to get min/max param index value", K(ret), K(obj_array_sorted_), K(params_.count()));
    } else {
      min_max_valid_ = true;
    }
  }
